...
- **Overall Accuracy**: 71.7% - indicating reliable identification of behavioral states
- **Precision**: 80.7% - demonstrating low false positive rates across behavioral categories  
- **Recall**: 61.5% - showing good ability to detect true instances of each behavioral state
...
### Per-Behavior Performance Analysis

**Resting Behavior (Excellent Performance)**:
- Precision: 97.3% - Nearly perfect precision with minimal false alarms
- Recall: 67.8% - Moderate recall, missing about 1/3 of resting periods
- The model excels at identifying resting with high confidence when predicted

**Foraging Behavior (Balanced Performance)**:
- Precision: 44.8% - Lower precision indicates some over-prediction
- Recall: 94.1% - Excellent sensitivity rarely missing foraging events
- Represents intermediate movement patterns that can overlap with other behaviors

**Migrating Behavior (Conservative Detection)**:
- Precision: 100% - Perfect precision with no false positives
- Recall: 22.7% - Conservative approach missing most migration events
- Reflects class imbalance and conservative classification of rare behaviors
...
**Class Imbalance Impact**: The natural distribution of behaviors (72% resting, 23% foraging, 5% migration) affects model performance, particularly for rare migration events. This biological reality requires careful interpretation of results and potential model adjustments for specific conservation applications.
